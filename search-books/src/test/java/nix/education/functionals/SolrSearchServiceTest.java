...
public class SolrSearchServiceTest {
    ...
    public class FindAllTest {
        ...
        public void shouldFindAllBooks() throws Exception {
            ...
            JsonNode responseBody = MAPPER
                .readTree(EntityUtils.toString(response.getEntity(), "UTF-8"));
    ...
    public class FindBooksTest {
        ...
        public void shouldFindBooksByDateQuery() throws Exception {
            HttpGet request =
                new HttpGet("http://localhost:8080/search?q=programming&fq=publication_date" +
                    "%3A%5B2014-12-31T22%3A00%3A00Z%20TO%202014-12-31T22%3A00%3A00Z%2B5YEAR%5D");
            HttpResponse response = httpClient.execute(request);

            JsonNode responseBody =
                MAPPER.readTree(EntityUtils.toString(response.getEntity(), "UTF-8"));
            JsonNode expectedBody = readFileContent("publication-query.json");
